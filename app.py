...
<style>
    ...
    .metric-card { background: linear-gradient(135deg, #1a0a2e, #2a1040);
    ...
    .result-box { background: linear-gradient(135deg, #1a0a2e, #3d1460);
...
with st.sidebar:
    ...
    labels = ["Inflammation","Fibrosis","Cirrhosis","End-Stage"]
    for i, (color, label) in enumerate(zip(stage_colors, labels), 1):
        ...
        if model is None or scaler is None or le_stage is None:
            ...
            try:
                ...
                if hasattr(model, 'predict_proba'):
                    ...
                    st.markdown("---")
                    st.markdown(f"#### {icon(ICO_CHART, 20, '#f3e5f5', '#6a1b9a')} Stage Probability Distribution", unsafe_allow_html=True)
                    ...
                    fig.update_layout(yaxis_title="Probability (%)", height=320,
                        margin=dict(t=20,b=60), plot_bgcolor='rgba(0,0,0,0)',
                        paper_bgcolor='rgba(0,0,0,0)', yaxis=dict(range=[0,110]))
...
# ══════════════════════════════════════════════════════════
    ...
    with col1:
        st.markdown(f"### {icon(ICO_TARGET, 20, '#f3e5f5', '#6a1b9a')} Objective", unsafe_allow_html=True)
        st.markdown("""
        Predict the **stage of liver cirrhosis** given clinical, biochemical, and personal patient data.
        Early detection of the stage can guide treatment plans and significantly improve patient outcomes.
        """)
        st.markdown(f"### {icon(ICO_LAYERS, 20, '#f3e5f5', '#6a1b9a')} Dataset Details", unsafe_allow_html=True)
        st.markdown("""
        | Property | Details |
        |---|---|
        ...
        """)
        st.markdown(f"### {icon(ICO_HEALTH, 20, '#f3e5f5', '#6a1b9a')} Normal Lab Value Ranges", unsafe_allow_html=True)
        for param, rng in NORMAL_RANGES.items():
            st.markdown(f"<div class='normal-range'><b>{param}:</b> {rng}</div>", unsafe_allow_html=True)

    with col2:
        st.markdown(f"### {icon(ICO_CPU, 20, '#f3e5f5', '#6a1b9a')} Methodology", unsafe_allow_html=True)
        st.markdown("""
        ...
        """)
        st.markdown(f"### {icon(ICO_CHART, 20, '#f3e5f5', '#6a1b9a')} Real-World Relevance", unsafe_allow_html=True)
        st.markdown("""
        - **Early Detection** — Identify cirrhosis stage for timely treatment
        - **Clinical Support** — Assist doctors with data-driven stage prediction
...
# ══════════════════════════════════════════════════════════
    ...
    col1, col2 = st.columns(2)
    with col1:
        st.markdown(f"### {icon(ICO_CPU, 20, '#f3e5f5', '#6a1b9a')} Preprocessing Pipeline", unsafe_allow_html=True)
        st.markdown("""
        | Step | Details |
        |---|---|
        ...
        """)
        st.markdown(f"### {icon(ICO_LAYERS, 20, '#f3e5f5', '#6a1b9a')} Why Scaling Matters", unsafe_allow_html=True)
        st.markdown("""
        ...
        """)
    with col2:
        st.markdown(f"### {icon(ICO_CHART, 20, '#f3e5f5', '#6a1b9a')} Feature Importance (Approximate)", unsafe_allow_html=True)
